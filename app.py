...
class QuizApp:
    ...
    def create_welcome_page(self):
        self.clear_window()
        ttk.Label(self.root, text=f"Welcome, {self.current_user.username}", style='Header.TLabel').pack(pady=20)
    ...
    def create_quiz_page(self, topic):
        self.clear_window()
        questions = self.quiz.get_questions(topic)
        score = [0]
        ...
        def next_question(index):
            if index < len(questions):
                ...
                self.create_result_page(score[0], len(questions), topic)

        def check_answer(selected, answer, next_index):
            if questions[next_index-1]["options"][selected] == answer:
                score[0] += 1
    ...
    def create_result_page(self, score, total, topic):
        self.clear_window()
        self.current_user.add_score(topic, score)
